...
> 一句话：在微信里扫码绑定一个「机器人好友」，把你和某个人的聊天记录交给我，我生成一个**像 ta 一样说话**的人设，然后你就可以在微信里像跟 ta 聊天一样继续聊下去。想干正事时，随时切到**任务模式**——一个与情感人设完全隔离的中性助手。
...
以下展示了从「生成人设」到「开始聊天」的关键步骤。

**① 进入项目文件夹，用 Claude Code 触发前任人设 skill**

在项目根目录打开终端，输入 `claude`，再输入 `/create-ex`：

![在项目根用 claude 触发 create-ex](docs/images/createex-step-terminal.png)

> 演示终端里的路径即本项目所在目录；Windows 用户直接双击 `设立人设.bat` 等效。

**② create-ex 开始采集：先给 ta 起个代号**

![create-ex 采集第一个问题：起代号](docs/images/createex-step-name.png)

之后按提示回答几个问题（基本信息、性格）即可，也可直接导入 `exports/` 里导出的聊天记录。

> 更多教学截图（微信绑定、PyWxDump 导出、导入聊天记录）已在本地 `docs/images/.private/`，里面含真实账号 / 昵称 / 对话，**打码后再放进公开仓库**。
